...
#' Normalise RT-qPCR CT values using the delta-CT method
#'
#' @param CTs data.frame with columns Target.Name, Sample.Name, CT
#' @param undetermined numeric replacement for "Undetermined" calls (default 40)
#' @param housekeeping.gene character vector of housekeeping gene name(s)
#' @param exclude_genes character vector of control gene names to exclude
#' @param handle.NAs logical; if TRUE imputes NAs near the observed minimum
#' @return numeric matrix (genes x samples) of 2^(-deltaCT) values
#' @export
extract.normalised.CTs <- function(CTs,
  ...
  if (is.null(housekeeping.gene)) {
    gapdh_var <- var(CTs$CT[CTs$Target.Name == "GAPDH"], na.rm = TRUE)
    hprt1_var <- var(CTs$CT[CTs$Target.Name == "HPRT1"], na.rm = TRUE)
...
#' Deprecated alias for extract.normalised.CTs
#' @inheritParams extract.normalised.CTs
#' @export
...
#' Extract raw CT values as a genes x samples matrix
#'
#' @param CTs data.frame with columns Target.Name, Sample.Name, CT
#' @param undetermined numeric replacement for "Undetermined" calls (default 40)
#' @return numeric matrix (genes x samples) of raw CT values
#' @export
